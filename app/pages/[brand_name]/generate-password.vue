      ...
      <v-card rounded="xl" elevation="8" width="420" class="pa-6 pa-sm-8">
        ...
        <div class="text-center mb-6">
          ...
          <p class="text-body-2 text-medium-emphasis">
            You might have generated password from this link or Your administrator has revoked your invitation. Please contact your Administrator for more info.
          </p>
        </div>
    ...
    <v-main class="d-flex align-center justify-center fill-height">
      <v-card rounded="xl" elevation="8" width="420" class="pa-6 pa-sm-8">
        ...
        <div class="text-center mb-6">
          ...
          <p class="text-body-2 text-medium-emphasis">You're generating password for {{ form.email }}</p>
        </div>
          ...
          <v-row no-gutters>
            <v-col cols="12" class="form-group required mb-4">
              ...
              <v-text-field
                ...
                @input="validateName"
              />
              <div v-if="nameError" class="form-controls-error">{{ nameError }}</div>
            </v-col>
...
const brand = computed(() => ({
  name: app.brand?.name || 'Digital Asset Manager',
  logo: app.brand?.logo || '',
...
const validatePassword = () => {
  if (!form.value.password) return (passwordError.value = 'Password is required')
  if (form.value.password.length < 6) return (passwordError.value = 'Password must be at least 6 characters')
  return (passwordError.value = null)
}

const validateConfirmPassword = () => {
  if (!form.value.confirm_password) return (confirmPasswordError.value = 'Confirm Password is required')
  if (form.value.confirm_password !== form.value.password) return (confirmPasswordError.value = 'Password and Confirm Password did not match.')
  return (confirmPasswordError.value = null)
}

const disableSubmitBtn = computed(() => {
  const hasErrors = !!nameError.value || !!passwordError.value || !!confirmPasswordError.value
  const emptyFields = !form.value.name.trim() || !form.value.password || !form.value.confirm_password
...
====================== */
const { data: invitationData } = await useAsyncData('invitation-data', async () => {
  ...
  try {
    ...
    if (err.data?.message?.includes('link is expired')) {
      ...
      throw createError({ statusCode: status, statusMessage: err.data?.message || err.message })
    }
  }
})
...
if (process.client && !error.value) {
  try {
    ...
    form.value.logo = appDataStore.brand?.branding?.brand_logo || ''
  } catch {}
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    snackbar.showSuccess(response.data?.message || response.data?.data?.message || response.message || 'Password generated successfully!')
